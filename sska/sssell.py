...
for i in range(1, lastpage):
	...
	for container in containers:
		...
		f.write ("http://ss.com" + con_link + "," + con_pic.replace(".th2.", ".800.") + ","  + con_addr.replace(",", ".").replace("А", "A").replace("а", "a").replace("к", "k").replace("с", "c").replace("ф", "f") + "," + con_rooms.replace(",", ".") + "," + con_sqrm + "," + con_floor + "," + con_type.replace(",", ".") + "," + con_monet.replace(",", "") + "\n")
