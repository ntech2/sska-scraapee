...
f.write(headers)
lastpage = input('Ievadi pedejaas lapas numuru - https://www.ss.com/lv/real-estate/flats/riga/all/hand_over/page55.html :::\n')
lastpage = int(lastpage)
lastpage = lastpage+1
for i in range(1, lastpage):
	k = str(i)
	my_url= 'https://www.ss.com/lv/real-estate/flats/riga/all/hand_over/page' + k + '.html'
	...
	for container in containers:
		...
		#te vajag salabot krilicu..tas kas te ir sadariits ar replace ir sliktii
		f.write ("http://ss.com" + con_link + "," + con_pic.replace(".th2.", ".800.") + ","  + con_addr.replace(",", ".").replace("А", "A").replace("а", "a").replace("к", "k").replace("с", "c").replace("К", "K").replace("В", "V").replace("и", "i").replace("е", "e").replace("н", "n").replace("и", "i").replace("б", "b").replace("а", "a").replace("с", "s")+  "," + con_rooms.replace(",", ".") + "," + con_sqrm + "," + con_floor + "," + con_type.replace(",", ".") + "," + con_monet.replace(",", "") + "\n")
...
f.close()
print("teh end! Ires sludinajumi saglabati")